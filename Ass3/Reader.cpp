...
bool Reader::get_record() {
	if (getline(in, current_input_line)) {
		...
		// update previous ship id to the previous current ship cuz we about to read a new ship yo.
		...
		auto i = 1;
		// counter for which variable to save current part of the line as. ex. each line has 5 pieces of information. we loop through the line
		// we have to know which piece of information to save now
		while ((pos = current_input_line.find(delimiter)) != string::npos) {
			// loop for the file
			auto token = current_input_line.substr(0, pos); // set our current token
			switch (i) {
				// based on which 5 pieces of information we're on now in the current line, that's what we save
			case 1: ship_id = ships.find(token)->second;
				break;
			case 2: departure_planet = planets.find(token)->second;
				break;
			case 3: departure_time = stoi(token);
				break;
			case 4: destination_planet = planets.find(token)->second;
				break;
			default: ;
			}
		...
		// at the end our current line/string is axed so all 4 former pieces of information are already saved in their respective 
